...
def markAttendance(name):
    ...
    # Check if it's been at least 1 minute since the last attendance
    if (name not in last_attendance_time) or \
            ((now - last_attendance_time[name]).total_seconds() >= 60):
        with open('Attendancesheet.csv', 'a', newline='') as f:
            writer = csv.writer(f)
